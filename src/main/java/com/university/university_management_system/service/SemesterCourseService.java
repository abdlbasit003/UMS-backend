...
public class SemesterCourseService {
    ...
    public List<SemesterCourseDTO> getAll(){
        ...
        for(SemesterCourseModel scm:  allCourses){
            courses.add(SemesterCourseDTO.fromModel(scm));
        }
        return courses;
    }
    public SemesterCourseDTO getBySemesterCourseId(int semesterCourseId){
        SemesterCourseModel scm = semesterCourseRepository.findById(semesterCourseId).orElseThrow(()->new ApiException("No semesterCourse found for semesterCourseId: "+ semesterCourseId, HttpStatus.NOT_FOUND));
    ...
    public List<SemesterCourseDTO> getBySemesterId(int semesterId){
      ...
      List<SemesterCourseDTO> semesterCourse = new ArrayList<>();
      for (SemesterCourseModel s: sc){
          semesterCourse.add(SemesterCourseDTO.fromModel(s));
      }
